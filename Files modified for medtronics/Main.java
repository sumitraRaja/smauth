       ...
       if (userServiceConfigured) {
		...
		service.login("", "", new AsyncCallback<Boolean>() {
			...
			private void showAccessDenied() {
				...
				RootLayoutPanel.get().setWidgetTopHeight(this.loginSats, 10, Unit.PCT,
						50, Unit.PCT);
			}
			...
			public void onSuccess(Boolean result) {
				if (result.booleanValue()) {
					...
					roleService.getRoleId(new AsyncCallback<String>() {
						...
						// not logged in, show the login dialog or Access Denied dialogue
						final RoleServiceAsync roleService = (RoleServiceAsync) GWT.create(RoleService.class);
						roleService.getUserId(new AsyncCallback<String>() {
								@Override
								public void onFailure(Throwable caught) {
									// ID and Role not available. User logged in through 8080 portal. Show xDB login dialogue 
									showLogin();
								}
								@Override
								public void onSuccess(String userId) {
